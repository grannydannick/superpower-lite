...
export type SelectorItem = {
  name: string;
  icon: LucideIcon;
  command: (editor: ReturnType<typeof useCurrentEditor>['editor']) => void;
  isActive: (editor: ReturnType<typeof useCurrentEditor>['editor']) => boolean;
};
...
export const NodeSelector = ({ open, onOpenChange }: NodeSelectorProps) => {
  ...
  return (
    <Popover modal={true} open={open} onOpenChange={onOpenChange}>
      <PopoverTrigger
        asChild
        className="gap-2 rounded-none border-none px-3 hover:bg-accent focus:ring-0"
      >
      ...
      <PopoverContent sideOffset={15} align="start" className="w-48 p-1">
        {items.map((item) => (
          <EditorBubbleItem
            key={item.name}
            onSelect={(editor) => {
